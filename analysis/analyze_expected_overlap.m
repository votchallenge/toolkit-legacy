...
%
    ...
    for i = 1:numel(trackers)
        ...
        if ~isempty(expected_overlaps) && ~isempty(evaluated_lengths)
            result.curves{i} = expected_overlaps;
            result.lengths = evaluated_lengths;
            continue;
        end;
        
        [expected_overlaps, evaluated_lengths] = estimate_expected_overlap(trackers{i}, experiment, experiment_sequences, 'Lengths', lengths);
