...
weights = 1 ./ occurences(context.sources);
...
for e = 1:numel(expected_overlaps)
    ...
    expected_overlaps(e) = sum(mean(fragments(usable, 2:len), 2) .* fweights(usable)) ./ sum(fweights(usable));
    practical_difference(e) = sum(mean(fpractical(usable, 2:len), 2) .* fweights(usable)) ./ sum(fweights(usable));
end
