...
//     fclose(FID);
//     MD5 = md5hash(S, 'unicode');  // 'unicode' can be omitted here
//
//   Test data:
//     md5hash(char(0:511), 'char', 'HEX')
//       => F5C8E3C31C044BAE0E65569560B54332
//     md5hash(char(0:511), 'unicode')
//       => 3484769D4F7EBB88BBE942BB924834CD
//
// Compile with:
//   mex -O md5hash.c
// On Linux the C99 comments must be considered (thanks Sebastiaan Breedveld):
//   mex -O CFLAGS="\$CFLAGS -std=C99" md5hash.c
//
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
  ...
  if (isFile) {
     if ((FileName = mxArrayToString(prhs[0])) == NULL) {
        mexErrMsgTxt("*** md5hash[mex]: Cannot get file name.");
     }
